...
function Follower() {
  let { currentUser, currentUserFollowerList, showLoader } = useSelector(
    (state) => state.auth
  );
  !currentUser.hasOwnProperty("_id") &&
    (currentUser = JSON.parse(localStorage.getItem("currentUser")));
  const dispatch = useDispatch();
  console.log({ currentUserFollowerList });
      ...
      <div className="w-full flex flex-col items-center ml-64 pl-1 pr-2">
        ...
        {showLoader ? (
          ...
          <div className="h-screen w-full flex flex-col items-center">
            {currentUserFollowerList.length > 0 ? currentUserFollowerList.map((user) => {
              return (
                <div
                  key={user._id}
